...
class FirebaseMethods implements AppMethods {
  ...
  try {
    FirebaseUser newUser = (await _firebaseAuth.createUserWithEmailAndPassword(email: email, password: password)).user;
  ...
  FirebaseUser user;
  try {
    FirebaseUser newUser = (await _firebaseAuth.signInWithEmailAndPassword(email: email, password: password)).user;
  if (user != null) {
